...
*
_arraydescr_try_convert_from_dtype_attr(PyObject
*obj)
{
return
_try_convert_from_dtype_attr(obj);
}
...
*
array_set_typeDict(PyObject
*
NPY_UNUSED(ignored), PyObject
*args)
{
PyObject *dict;
...
_convert_from_array_descr(PyObject *obj, int align) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        PyTuple_SET_ITEM(tup, 0, (PyObject * )
        conv);
...
_convert_from_field_dict(PyObject *obj, int align) {
    ...
    res = (PyArray_Descr * )
    PyObject_CallMethod(_numpy_internal,
...
_convert_from_any(PyObject *obj, int align) {
    ...
    if (obj == Py_None) {
        ...
        PyArray_Descr * ret = (PyArray_Descr * )
        obj;
        Py_INCREF(ret);
        return ret;
...
newdescr->
c_metadata = NPY_AUXDATA_CLONE(base->c_metadata);
...
newdescr->
subarray = PyArray_malloc(sizeof(PyArray_ArrayDescr));
if (newdescr->subarray == NULL) {
Py_DECREF(newdescr);
return (PyArray_Descr *)
...
memcpy(newdescr
->subarray, base->subarray, sizeof(PyArray_ArrayDescr));
Py_INCREF(newdescr
->subarray->shape);
Py_INCREF(newdescr
->subarray->base);
...
NPY_NO_EXPORT PyObject
*
arraydescr_protocol_typestr_get(PyArray_Descr
*self,
...
return
PyLong_FromLong(ndim);
}


NPY_NO_EXPORT PyObject
*
arraydescr_protocol_descr_get(PyArray_Descr
*self,
...
PyTuple_SET_ITEM(dobj,
0, PyUnicode_FromString(""));
PyTuple_SET_ITEM(dobj,
1,
arraydescr_protocol_typestr_get(self, NULL
));
res = PyList_New(1);
if (res == NULL) {
Py_DECREF(dobj);
...
{
if (
PyDataType_FLAGCHK(self, NPY_ITEM_HASOBJECT
)) {
Py_RETURN_TRUE;
}
else {
...
PyTuple_SET_ITEM(ret,
1, Py_BuildValue("(NOO)", obj, Py_False, Py_True));
...
PyTuple_SET_ITEM(state,
1, PyUnicode_FromFormat("%c", endian));
PyTuple_SET_ITEM(state,
2,
arraydescr_subdescr_get(self, NULL
));
if (
PyDataType_HASFIELDS(self)
) {
...
PyTuple_SET_ITEM(state,
5,
PyLong_FromLong(elsize)
);
PyTuple_SET_ITEM(state,
6,
PyLong_FromLong(alignment)
);
PyTuple_SET_ITEM(state,
7,
...
arraydescr_setstate(PyArray_Descr *self, PyObject *args) {
    ...
    if (subarray != Py_None) {
        ...
        self->subarray->base = (PyArray_Descr * )
        PyTuple_GET_ITEM(subarray, 0);
...
NPY_NO_EXPORT PyArray_Descr
*
PyArray_DescrNewByteorder(PyArray_Descr
*self,
char newendian
)
{
...
while (
PyDict_Next(self
->fields, &pos, &key, &value)) {
if (
NPY_TITLE_KEY(key, value
)) {
continue;
}
if (!
...
if (
PyDataType_HASSUBARRAY(new)
) {
Py_DECREF(new
->subarray->base);
new->subarray->
base = PyArray_DescrNewByteorder(
...
arraydescr_newbyteorder(PyArray_Descr *self, PyObject *args) {
    ...
    return (PyObject * )
    PyArray_DescrNewByteorder(self, endian);
}
...
descr_repeat(PyObject *self, Py_ssize_t length) {
    ...
    if (length < 0) {
        return PyErr_Format(PyExc_ValueError,
                            "Array length must be >= 0, not %"
        NPY_INTP_FMT, (npy_intp) length);
    }
    tup = Py_BuildValue("O"
    NPY_SSIZE_T_PYFMT, self, length);
    if (tup == NULL) {
        return NULL;
    }
...
NPY_NO_EXPORT PyArray_Descr
*
arraydescr_field_subset_view(PyArray_Descr
*self,
PyObject *ind
)
{
...
descr_subscript(PyArray_Descr *self, PyObject *op) {
    ...
    if (PyUnicode_Check(op)) {
        ...
        return (PyObject * )
        arraydescr_field_subset_view(self, op);
    } else {
